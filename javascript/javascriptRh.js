$(document).ready(function () {
    ...
    function createForm() {
        ...
        html = html + '<label class="control-label" for="cod_sede">Sede</label>';
        html = html + '<input type="text" name="cod_sede" class="form-control" data-error="Por favor entre com o código da sede" required />';
        html = html + '<div class="help-block with-errors"></div>';
        html = html + '</div>';
        html = html + '<div class="form-group">';
        html = html + '<label class="control-label" for="nome_rh">Nome</label>';
        html = html + '<input type="text" name="nome_rh" class="form-control" data-error="Por favor entre com o nome" required />';
        html = html + '<div class="help-block with-errors"></div>';
        html = html + '</div>';
        html = html + '<div class="form-group">';
        html = html + '<label class="control-label" for="cpf_rh">CPF</label>';
        html = html + '<input type="text" name="cpf_rh" class="form-control" data-error="Por favor entre com o CPF" required />';
        html = html + '<div class="help-block with-errors"></div>';
        html = html + '</div>';
        html = html + '<div class="form-group">';
        html = html + '<label class="control-label" for="email_rh">E-mail</label>';
        html = html + '<input type="text" name="email_rh" class="form-control" data-error="Por favor entre com o e-mail" required />';
        html = html + '<div class="help-block with-errors"></div>';
        html = html + '</div>';
        html = html + '<label class="control-label" for="tel_rh">Telefone</label>';
        html = html + '<input type="text" name="tel_rh" class="form-control" data-error="Por favor entre com o telefone" required />';
        html = html + '<div class="help-block with-errors"></div>';
        html = html + '</div>';
        html = html + '<div class="form-group">';
    ...
    $(".crud-submit").click(function (e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            url: form_action,
            data: {cod_sede: cod_sede, nome_rh: nome_rh, cpf_rh: cpf_rh, email_rh: email_rh, tel_rh: tel_rh}
        }).done(function (data) {
